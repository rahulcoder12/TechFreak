...
userSchema.method('confirmAccount', async function () {
  try {
    ...
    await sendEmail(
      this.Email,
      'Tech-Freak needs you to verify your email address', // <-- Changed Subject
      returnHtml(
        ...
        <a style ="color:#333; text-decoration:none;font-size:18px; display:inline-block; border-radius:9px; color:#fff; padding:0.9rem 1.4rem; background-color:#1c7ed6" href="http://localhost:5173/users/activate/${token}">Confirm Account</a> 
        <br> <br>  Note that this link is valid for only 10minutes. <br> <br>Once you have confirmed your email, you will be able to access all of the features of our website and start shopping! <br> <br>  If you didn't initiate this request, <br> kindly disregard this email. <br> <br> Regards, <br> <br> Tech-Freak Team` // <-- Changed Signature
      )
    );
  } catch (err) {
    if (err.errno === -3008)
...
userSchema.method('forgotPassword', async function () {
  ...
  await sendEmail(
    this.Email,
    'Reset your Tech-Freak password', // <-- Changed Subject
    returnHtml(
      `Dear ${this.Name}, <br> <br>  We have received a request to reset your password for your account. If you did not request a password reset, please ignore this email. <br> <br> To reset your password, please click on the link below: <br> <br> <a style ="color:#333; text-decoration:none;font-size:18px; display:inline-block; border-radius:9px; color:#fff; padding:0.9rem 1.4rem; background-color:#1c7ed6" 
      href="http://localhost:5173/users/reset-password/${token}?id=${cryptr.encrypt(
        this.Email
      )}">Reset Password</a> <br><br> If the link above does not work, you can also copy and paste the URL into your browser. <br> <br> Note that this link is valid for 10minutes. <br> <br> After the time limit has expired, you will have to resubmit the request for a password reset <br><br> Regards, <br> <br> Tech-Freak Team.` // <-- Changed Signature
    )
  );
});
